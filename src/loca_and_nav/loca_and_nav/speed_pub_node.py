...
f_c_default = 1.9750       # drag coefficient
...
class Speed_Pub(Node):
    ...
    def __init__(self):
        ...
        self.F_spring_damper_subscription = self.create_subscription(Vector3Stamped, F_spring_damper_force_topic, self.F_spring_damper_update, 10)
    ...
    def loca_pub(self):
        ...
        if self.enable_obstacle_force:
            ...
            if vel.linear.x == 0.0:
                if self.F_y >= 0: 
                    vel.angular.z = (self.F_y + np.absolute(self.F_x)) * self.K_w
                else:
                    vel.angular.z = (self.F_y - np.absolute(self.F_x)) * self.K_w
            else:
                vel.angular.z = self.F_y/vel.linear.x
